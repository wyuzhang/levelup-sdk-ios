...
SPEC_BEGIN(LULocationV14JSONFactorySpec)

describe(@"LULocationV14JSONFactory", ^{
  __block LULocationV14JSONFactory *factory;

  beforeEach(^{
    factory = [LULocationV14JSONFactory factory];
  });

  describe(@"createFromAttributes:", ^{
    it(@"parses a JSON dictionary into an LULocation", ^{
      LULocation *location = [factory createFromAttributes:[LULocation fullV14JSONObject]];
