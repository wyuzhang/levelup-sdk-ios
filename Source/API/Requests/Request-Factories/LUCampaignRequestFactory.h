...
/**
 Builds a request to retrieve the merchants associated with the campaign. Only the merchant IDs are returned; they are
 represented as an `NSArray` of `NSNumber` instances. If the campaign is global (it applies to all merchants), then this
 request will return `nil`.
...
/**
 Builds a request to retrieve a campaign by its code. This is most commonly used when a user has scanned a QR code for
 the campaign. The value retrieved from the QR code can be used with this request to retrieve the associated campaign.
